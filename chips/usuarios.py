...
def novo_usuario():
    with pyodbc.connect(conn_str) as conn:
        ...
        if request.method == 'POST':
            ...
            cursor.execute("""
                INSERT INTO Usuarios (Nome, Email, EstadoPadraoId)
                VALUES (?, ?, ?)
            """, nome, email, estado_id)
            conn.commit()
            return redirect(url_for('usuarios.listar_usuarios'))

        estados = cursor.execute("SELECT Id, Nome FROM Estados").fetchall()
        return render_template('chips/usuarios_novo.html', estados=estados)